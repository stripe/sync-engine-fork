...
export async function resolveOpenApiSpec(config: ResolveSpecConfig): Promise<ResolvedOpenApiSpec> {
  ...
  // The CDN serves every published spec version without auth or rate limits.
  const cdnSpec = await tryFetchFromCdn(apiVersion)
  if (cdnSpec) {
...
async function tryFetchFromCdn(apiVersion: string): Promise<OpenApiSpec | null> {
  ...
  try {
    ...
    const specRes = await fetchWithProxy(specUrl)

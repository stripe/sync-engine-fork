 ...
 *
 * Usage:
 *   pnpm tsx scripts/explorer-export.ts [--output-dir=path/to/dir]
 *
 * Flags:
 *   --output-dir   Output directory for artifacts (default: packages/visualizer/public/explorer-data)
 */
...
const DEFAULT_OUTPUT_DIR = path.join(process.cwd(), 'packages/visualizer/public/explorer-data')
...
function parseArgs(): { outputDir: string } {
  ...
  for (const arg of args) {
    if (arg.startsWith('--output-dir=')) {
      ...
      console.log('Usage: pnpm tsx scripts/explorer-export.ts [--output-dir=path/to/dir]')
      console.log('')
      console.log('Flags:')
      console.log('  --output-dir   Output directory for artifacts (default: packages/visualizer/public/explorer-data)')
      process.exit(0)
    } else {
 ...
 */
async function exportDatabase(client: Client, outputDir: string): Promise<void> {
  ...
  const bootstrapFile = path.join(outputDir, 'bootstrap.sql')
  fs.writeFileSync(bootstrapFile, sql)
  const sqlSizeKB = Math.round((sql.length / 1024) * 10) / 10
  console.log(`   ✓ Written ${sqlSizeKB} KB to ${bootstrapFile}`)
  ...
  console.log(`   ✓ Written to ${manifestFile}`)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await exportDatabase(client, config.outputDir)

...
class StripeDataGraph {
  ...
  private async seedPrices(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const productId = this.gen.productId((i % 40) + 1)
  ...
  private async seedPaymentMethods(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      ...
      const rawData = {
        ...
        billing_details: {
          address: null,
          email: this.gen.email((i % 125) + 1),
          name: this.gen.name((i % 125) + 1),
          phone: null,
        },
        card: {
  ...
  private async seedSetupIntents(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      const paymentMethodId = this.gen.paymentMethodId((i % 150) + 1)
  ...
  private async seedSubscriptionItems(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const subscriptionId = this.gen.subscriptionId((i % 100) + 1)
      const priceId = this.gen.priceId((i % 60) + 1)
  ...
  private async seedInvoices(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      const subscriptionId = i <= 100 ? this.gen.subscriptionId(i) : null
      const created = this.gen['rng'].nextTimestamp()
      ...
      const rawData = {
        ...
        customer_email: this.gen.email((i % 125) + 1),
        customer_name: this.gen.name((i % 125) + 1),
        customer_phone: null,
  ...
  private async seedPaymentIntents(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      const invoiceId = i <= 175 ? this.gen.invoiceId(i) : null
      const paymentMethodId = this.gen.paymentMethodId((i % 150) + 1)
      const amount = this.gen.amount()
  ...
  private async seedCharges(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      const invoiceId = i <= 175 ? this.gen.invoiceId(i) : null
      const paymentIntentId = i <= 200 ? this.gen.paymentIntentId(i) : null
      const paymentMethodId = this.gen.paymentMethodId((i % 150) + 1)
      const amount = this.gen.amount()
  ...
  private async seedRefunds(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const chargeId = this.gen.chargeId((i % 225) + 1)
      const paymentIntentId = this.gen.paymentIntentId((i % 200) + 1)
      const amount = this.gen.amount()
  ...
  private async seedCreditNotes(client: Client): Promise<void> {
    ...
    for (let i = 1; i <= count; i++) {
      const customerId = this.gen.customerId((i % 125) + 1)
      const invoiceId = this.gen.invoiceId((i % 175) + 1)
      const amount = this.gen.amount()
...
class GenericTableSeeder {
  ...
  async seedTable(client: Client, table: ParsedResourceTable): Promise<number> {
    // Determine row count (5-100 for long-tail tables)
    const rowCount = this.rng.nextInt(5, 100)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Parse the GET-retrievable projected table superset.
